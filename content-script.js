...
function fetchAttributes() {
  ...
  if (pageref && contentType && locale) {
    ...
    return nodeAttr
      ? [
          nodeAttr.getAttribute("data-pageref"),
          nodeAttr.getAttribute("data-contenttype"),
          nodeAttr.getAttribute("data-locale"),
        ]
      : "";
  }
}
...
function checkDomain(stack, btn, btnPos) {
  ...
  if (stacks.length > 1) {
    stacks.forEach((stack, idx) => {
      const domain = domains[idx].map((d) =>
        d.toString().replace(/[`~!@#$%^&*()|+\=?;'",<>\{\}\[\]\\\/]/gi, "")
      );
      let csHost;
...
observer = new MutationObserver(function () {
  let bodyAttr = fetchAttributes();
  const editButton = document.getElementsByClassName('ext__cms__edit');

  if (editButton.length > 0) { 
  const newUrl = new URL(editButton[0].href);
  const splitedUrl = newUrl.hash.split('content-type/')[0];
  const entryUrl = `content-type/${bodyAttr[1]}/${bodyAttr[2]}/entry/${bodyAttr[0]}/edit`
  editButton[0].href = newUrl.origin+splitedUrl+entryUrl;
}
...
observer.observe(document.body, {
  ...
  childList: true,
  subtree: true,
  attributeFilter: ["data-contenttype", "data-pageref"],
});
